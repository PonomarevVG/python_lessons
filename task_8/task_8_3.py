...
number_list = []
while True:
    s = input('Вводите числа (для завершения введите "stop"): ')
    if s.lower() == 'stop':
        break
    try:
        if not s.replace('.', '').isdigit():
            raise ListElemError('В список могут входить только числа!')
        number_list.append(float(s))
    except ListElemError as err:
        print(err.txt)
